...
## ライセンス
* phalcon及びミドルウェアを除いた本フレームワークはMITライセンスとなります。
* 本プロジェクトを使用した損害について一切責任を負いません。自己責任でお願いします。
...
* 基本 $ su - の root でインストール進めます。# は root $ はユーザー操作です。
...
## memcached / redis への限界点

* DAU100Krpsとか超えてくると、DBのシャーディング以外にもNoSQL系のシャーディングに対応が必要になる。sidからハッシュ関数(s mod n)でmemcached / redis へのアクセスを水平分割する事も必要となるだろう。
